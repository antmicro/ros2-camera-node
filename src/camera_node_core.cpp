...
CameraNode::parameters_set_callback(const std::vector<rclcpp::Parameter> &parameters)
{
    ...
    for (const auto &parameter : parameters)
    {
        ...
        {
            camera->setFormat(static_cast<uint32_t>(parameter.as_integer_array()[0]),
                              static_cast<uint32_t>(parameter.as_integer_array()[1]));
        }
        else if (parameter.get_name() == "camera_refresh_rate")
        {
            camera_frame_clock->cancel();
            camera_frame_clock = create_wall_timer(std::chrono::duration<double>(1 / parameter.as_double()),
                                                   std::bind(&CameraNode::camera_frame_callback, this));
        }
        else if (parameter.get_name() == "camera_info_rate")
        {
            camera_frame_info_clock->cancel();
            camera_frame_info_clock = create_wall_timer(std::chrono::duration<double>(1 / parameter.as_double()),
...
void CameraNode::camera_get_properties_callback(const camera_node::srv::CameraGetProperties::Request::SharedPtr request,
...
void CameraNode::generate_mat_mappings()
{
    ...
    for (auto &[cvtype, prefix] :
         {std::tuple<int, std::string>{CV_8U, "8U"}, std::tuple<int, std::string>{CV_8S, "8S"},
          std::tuple<int, std::string>{CV_16U, "16U"}, std::tuple<int, std::string>{CV_16S, "16S"},
          std::tuple<int, std::string>{CV_32S, "32S"}, std::tuple<int, std::string>{CV_32F, "32F"},
...
CameraNode::CameraNode(const rclcpp::NodeOptions &options) : Node("camera_node", options)
{
    ...
    {
        camera->setFormat(static_cast<uint32_t>(get_parameter("camera_frame_dim").as_integer_array()[0]),
    ...
    camera_get_property_details_srv = create_service<camera_node::srv::CameraGetPropertyDetails>(
        "camera_get_property_details", std::bind(&CameraNode::camera_get_property_details_callback, this,
                                                 std::placeholders::_1, std::placeholders::_2));

    camera_frame_pub = create_publisher<sensor_msgs::msg::Image>("camera_frame", 1);

    camera_frame_clock =
        create_wall_timer(std::chrono::duration<double>(1 / get_parameter("camera_refresh_rate").as_double()),
                          std::bind(&CameraNode::camera_frame_callback, this));

    camera_frame_loopback = create_subscription<sensor_msgs::msg::Image>(
        "camera_frame", 10, std::bind(&CameraNode::camera_frame_loopback_callback, this, std::placeholders::_1));

    camera_frame_info_clock =
        create_wall_timer(std::chrono::duration<double>(1 / get_parameter("camera_info_rate").as_double()),
                          std::bind(&CameraNode::camera_frame_info_callback, this));
}
